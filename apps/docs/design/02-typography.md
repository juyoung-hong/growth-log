...
### 4. tailwind-merge에 이 스케일을 등록해 둔다 — 안 그러면 색이 조용히 사라진다

**뒤늦게 발견한 문제**: `text-11`~`text-30`은 Tailwind 기본 스케일이 아니라서, `shared/lib/utils.ts`의 `cn()`(=`tailwind-merge`)이 이 숫자들을 "글자 크기"로 인식하지 못한다. 그러면 `text-14`처럼 인식 못 하는 숫자 접미사를, `text-grey-700`같은 글자 색과 **같은 애매한 그룹**으로 묶어버린다 — 크기와 색을 한 문자열에서 같이 쓰면 twMerge가 그중 하나를 "충돌"로 보고 조용히 지워버린다는 뜻이다.

실제로 이 문제 때문에 `DialogDescription`·`AlertDescription`·`CardDescription`·`TableCaption` **네 컴포넌트가 전부 `text-muted-foreground`(옅은 회색 글자색)를 렌더링하지 못하고 있었다** — 베이스 클래스 문자열 안에서 색이 크기보다 먼저 쓰여 있어서, twMerge가 뒤에 오는 `text-14`를 "이긴 값"으로 보고 색 쪽을 버렸다. 화면이 완전히 깨지는 게 아니라 글자색만 브라우저 기본값으로 조용히 바뀌는 정도라 눈치채기 어려웠다.
...
`theme.text`에 이 숫자들을 등록하면 twMerge가 "이건 글자 크기 그룹"이라고 정확히 알게 되고, 색 그룹과 더는 섞이지 않는다. **커스텀 스케일로 완전히 갈아탄 프로젝트라면, `cn()`이 쓰는 병합 라이브러리에도 그 스케일을 반드시 등록해야 한다** — 기본 스케일(`text-sm`/`text-base`)만 아는 라이브러리는 그 바깥의 모든 숫자를 못 미더운 값으로 취급한다.
...
7. Tailwind 기본 스케일을 리셋하고 커스텀 숫자 스케일을 새로 등록했다면, `tailwind-merge`(또는 다른 클래스 병합 라이브러리)에도 그 스케일을 반드시 알려준다. 안 그러면 크기와 색처럼 서로 다른 성격의 유틸리티가 조용히 서로를 지워버릴 수 있다 — 위 4번 참고.
